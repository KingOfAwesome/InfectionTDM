...
namespace UI
{
	namespace MapPreview
	{
		...
		void Render( Proxy@ proxy ){
			...
			string filename = "MiddleIsland.png";//TitleBackground MenuItems Princess07 MiddleIsland
			float scale;
			proxy.control.vars.get("scale", scale);
			Vec2f dim, pos, mapOffset, origin, offset, size;
			
			size = proxy.lr - proxy.ul - Vec2f(22, 22);
			proxy.control.vars.get("map offset", mapOffset);
			GUI::GetImageDimensions(filename, dim);
			...
			GUI::DrawIconDirect(filename, pos, offset, dim, scale/2, 0, color_white);
