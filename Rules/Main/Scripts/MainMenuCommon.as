...
void RenderTitleBackgound( UI::Proxy@ proxy )
{
	...
	float scale = getScreenWidth()/1024.0;
	GUI::DrawIcon("TitleBackground", 0, Vec2f(1024, 830), Vec2f(0, (getScreenHeight()-830.0*scale)/2), scale/2);
	GUI::DrawIcon("title", 0, Vec2f(512, 128), Vec2f(0, 0), scale);
	//print(""+getScreenWidth()/2048.0);
}
...
void SelectChallenges( UI::Group@ group, UI::Control@ control ){
	LoadMapCycle(  "Challenge/mapcycle.cfg" );
	LoadRules("Rules/Challenge/gamemode.cfg");
	LoadNextMap();
}

void SelectSandbox( UI::Group@ group, UI::Control@ control ){
	LoadMapCycle(  "Sandbox/mapcycle.cfg" );
...
void ShowConnectTo( UI::Group@ group, UI::Control@ control )
{
	...
	UI::AddGroup("Connect to... 1", Vec2f(0.30,0.3), Vec2f(0.72,0.7));
		UI::Grid( 1, 3 );
		UI::TextInput::Add("IP:Port", SetIP, sv_ip + ":" + sv_port);
		UI::TextInput::Add("Password", SetPass, sv_password);
...
string SetIP( const string &in caption )
{
	...
	if (split.length != 2)
		return sv_ip + ":" + sv_port;
	
	sv_ip = split[0];
	sv_port = parseInt(split[1]);
	return caption;
}

string SetPass( const string &in caption )
{
	return sv_password = caption;
}

void ConnectTo( UI::Group@ group, UI::Control@ control )
{
	getNet().Connect(sv_ip, sv_port);
}
...
void ShowPlayerMenu( UI::Group@ group, UI::Control@ control )
{
	...
	UI::AddGroup(PLAYER_SETTINGS_STRING, Vec2f(0.2,0.3), Vec2f(0.6,0.9));
		...
		UI::Option::Add( "Gender:", SetGender, "Male|Female|Geti|Geti|Geti", cl_sex, "Your character's gender" );
...
void SetupFilter()
{
	getBrowser().filter = 
		 ...
		 (g_filtermode != "All" ? "/gameMode/"  + g_filtermode : "") +
	...
	for (int i = 0; i < allServers.length; ++i)
		if ((g_sort != 5 || isFavourite(allServers[i]))
			&& (search == "" 
				...
				|| allServers[i].serverIPv6Address.find(search) != -1
...
float ScrollServers( float newValue ) 
{
	...
	UI::Control@ prev;
	if(getRules().get("radio set servers selection", @prev)){
...
{
	UI::Control@ selected;
	if(getRules().get("radio set servers selection", @selected)){
		APIServer@ s;
		selected.vars.get( "server", @s );
		getNet().DisconnectClient();
		getNet().DisconnectServer();
		getNet().Connect(s.serverIPv4Address, s.serverPort);
		// print("connect: "+s.serverIPv4Address+":"+s.serverPort);
	}
}
